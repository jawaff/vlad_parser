...
from vlad.parser.parser import Grammar, TokenMatcher
...
class Rule:
    ...
    def __init__(
        self, type: RuleType, id: str, token_id: int, matchers: List[TokenMatcher]
    ):
        self.type = type
        self.id = id
...
class VLADParserVisitor(ParseTreeVisitor):
    ...
    def build_grammar(self) -> Grammar:
        ...
        root_matcher = LiteralTokenMatcher(
            {self._rules[self._root_rule_id].token_id: None}, False, False
        )
        # Note that fragment rules should not exist as a standalone rule in the grammar.
        rules = {
            rule.token_id: rule.matchers
    ...
    def _create_new_token(self, token: str) -> int:
        ...
        self._translator.add_new_token(
            self._prepare_token_id(token), error_on_exists=True
        )
    ...
    def _create_junction_rule(self, partial_rules: List[PartialRule]) -> TokenMatcher:
        ...
        for partial_rule in partial_rules:
            ...
            self._rules[token] = Rule(
                RuleType.JUNCTION, token, token_id, partial_rule.matchers
            )
    ...
    def _literal_to_rule(self, literal: str) -> PartialRule:
        ...
        if literal in self._rules:
            ...
            if target_rule.type == RuleType.FRAGMENT:
                ...
                matchers = [
                    LiteralTokenMatcher(
                        {self._rules[literal].token_id: None}, False, False
                    )
                ]
        else:
            if not (literal.startswith("'") and literal.endswith("'")):
                raise Exception(
                    f"Rule reference must come after rule definition: {literal}"
                )
            # We need to cut off the last id because it should be a 1.
            ids = self._translator.translate(literal)
            assert ids[-1] == 1
            matchers = [
                LiteralTokenMatcher({id: None}, False, False) for id in ids[0:-1]
            ]
    ...
    def _regex_literal_to_rule(self, regex: str) -> PartialRule:
        ...
        if len(token_ids) == 0:
            raise Exception(
                f"Regex did not match a single token in the model's vocabulary: {regex}"
            )
    ...
    def visitRootTokenRule(self, ctx: VLADParser.RootTokenRuleContext):
        ...
        self._rules[self._cur_rule_id] = Rule(
            type=RuleType.TOKEN, id=self._cur_rule_id, token_id=-1, matchers=[]
        )
        return self.visitChildren(ctx)
    ...
    def visitSpecialRule(self, ctx: VLADParser.SpecialRuleContext):
        self._cur_rule_id = ctx.ID().getText()
        self._rules[self._cur_rule_id] = Rule(
            type=RuleType.SPECIAL, id=self._cur_rule_id, token_id=-1, matchers=[]
        )
        return self.visitChildren(ctx)
    ...
    def visitFragmentRule(self, ctx: VLADParser.FragmentRuleContext):
        self._cur_rule_id = ctx.ID().getText()
        self._rules[self._cur_rule_id] = Rule(
            type=RuleType.FRAGMENT, id=self._cur_rule_id, token_id=-1, matchers=[]
        )
    ...
    def visitElement(self, ctx: VLADParser.ElementContext) -> PartialRule:
        ...
        if suffix is not None:
            if len(rule.matchers) == 0:
                raise Exception(
                    "An ebnf suffix must be applied to a matcher, but there is none"
                )
